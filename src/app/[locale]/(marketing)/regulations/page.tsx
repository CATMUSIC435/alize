...
/**
 * Generates SEO metadata for the Operating Regulations page.
 * @param props Page parameters promise.
 * @returns Metadata object.
 */
export async function generateMetadata(props: RegulationsPageProps): Promise<Metadata> {
  const { locale } = await props.params;
  const t = await getTranslations({ locale, namespace: 'RegulationsPage' });
  const alternates = getI18nAlternates('/regulations', locale);
  ...
  return {
    title,
    description,
    keywords: getLocalizedKeywords('regulations', locale),
    alternates,
    openGraph: {
      title,
      description,
...
/**
 * Dedicated Operating Regulations & Governance content page.
 * @param props Page parameters promise.
 * @returns The RegulationsPage component.
 */
export default async function RegulationsPage(props: RegulationsPageProps) {
  const { locale } = await props.params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: 'RegulationsPage' });
  const baseUrl = getBaseUrl();
  const routePath = '/regulations';
      ...
      <main className="bg-textured-sand relative min-h-screen w-full pt-[16vh] pb-24 md:pt-[20vh] md:pb-36">
        ...
        <article className="relative z-10 mx-auto w-full max-w-[1000px] px-6 md:px-12">
          ...
          <div className="mb-6 flex items-center gap-2 text-[9px] font-bold tracking-[0.2em] text-[#151926]/60 uppercase sm:text-[10px]">
            ...
            <span className="text-[#8B7043]">{t('breadcrumb_regulations')}</span>
          </div>
