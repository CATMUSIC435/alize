...
/**
 * Renders the luxury localized 404 error page matching Alizé Residence design language.
...
export default function NotFoundPage() {
  ...
  return (
    <main
      className={`${inter.className} relative flex min-h-screen w-full flex-col items-center justify-between overflow-hidden bg-[#0A1926] px-4 py-6 text-[#151926] select-none sm:px-8 sm:py-10`}
    >
      {/* Background Decorative Ocean & Sand Atmosphere */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {/* Soft Golden Sand Ambient Glow */}
        <div className="absolute -top-40 left-1/2 h-[600px] w-[600px] -translate-x-1/2 rounded-full bg-[#E0AC87]/15 blur-[130px]" />

        {/* Deep Mediterranean Blue Central Glow */}
      ...
      {/* Header Bar: Brand Logo & Navigation */}
      <motion.header
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
        className="relative z-10 flex w-full max-w-6xl items-center justify-between pt-2"
      >
        <Link
          href="/"
          prefetch={false}
        ...
        >
          ...
          <div className="flex flex-col text-left">
            <span
              className={`${playfair.className} text-base font-medium tracking-[0.24em] text-[#FAF8F5] sm:text-lg`}
            >
              ALIZÉ
            </span>
        ...
        <nav className="flex items-center gap-4 text-[11px] font-medium tracking-[0.2em] uppercase sm:gap-8 sm:text-xs">
          <Link
            href="/"
            className="text-[#FAF8F5]/70 transition-colors hover:text-[#E0AC87]"
          >
            Trang chủ
          </Link>
          <Link
          ...
          <Link
            href="/contact"
            className="hidden text-[#FAF8F5]/70 transition-colors hover:text-[#E0AC87] sm:inline-block"
          >
            Liên hệ
          </Link>
        </nav>
      ...
      {/* Center Content: Luxury Alizé Sand Card */}
      <motion.div
        initial={{ opacity: 0, scale: 0.95, y: 24 }}
        animate={{ opacity: 1, scale: 1, y: 0 }}
        transition={{ duration: 0.9, delay: 0.15, ease: [0.16, 1, 0.3, 1] }}
        className="relative z-10 my-auto flex w-full max-w-xl flex-col items-center py-6"
      >
        {/* Signature Alizé Sand Card */}
        <div className="bg-sand-card border border-[#D9CEBD]/90 relative flex w-full flex-col items-center rounded-2xl p-6 shadow-[0_24px_80px_rgba(0,0,0,0.45)] sm:p-10 md:p-12 text-center">
          ...
          <div className="relative z-10 flex w-full flex-col items-center">
            ...
            {/* Stylized 404 Hero Number */}
            <h1
              className={`${playfair.className} text-7xl font-semibold tracking-[0.14em] text-[#151926] sm:text-8xl md:text-9xl`}
            >
              {t('code')}
            </h1>
            ...
            {/* Main Title */}
            <h2
              className={`${playfair.className} mb-3 text-2xl font-semibold tracking-wide text-[#151926] uppercase sm:text-3xl`}
            >
              {t('title')}
            </h2>
            ...
            >
              <div className="relative flex h-6 w-6 items-center justify-center">
                ...
                <span className={`${playfair.className} absolute text-[10px] font-bold text-[#7D5C2C]`}>
                  {countdown}
                </span>
              </div>
      ...
      <motion.footer
        ...
        className="relative z-10 flex flex-col items-center gap-1 text-center pb-2"
      >
