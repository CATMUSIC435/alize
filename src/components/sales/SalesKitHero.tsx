...
type SalesKitHeroProps = {
  itemCount?: number;
};

/**
 * Hero section for Sales Kit page with flowing sand ripples and aesthetic wave dividers.
 * @param props Hero properties including item count.
 * @returns The SalesKitHero component.
 */
export function SalesKitHero(props: SalesKitHeroProps) {
  const t = useTranslations('SalesKitPage');
  const count = props.itemCount ?? 6;
    ...
    <section className="relative w-full overflow-hidden bg-transparent pt-[16vh] pb-[4vh] md:pt-[20vh] md:pb-[6vh]">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="mb-6 flex items-center gap-2 text-[9px] font-bold tracking-[0.2em] text-[#151926]/60 uppercase sm:text-[10px]">
          ...
          <span className="text-[#8B7043]">{t('breadcrumb_sales')}</span>
        </motion.div>
